...
| P3 | 查询执行 | 13 个算子（含窗口函数）+ 3 条优化器规则 | ✅ 21/21 | [04-P3-执行引擎](./04-P3-execution.md) |
...
`p3.20-window-function.slt` 已通过（STEP 26–28）。要点见
[04-P3 第 7 节](./04-P3-execution.md)：它是**最彻底的阻塞算子**（内存 O(行数)），
以及那个测试没覆盖到的坑——SQL 默认窗口帧是 RANGE 而非 ROWS，
**排序键相同的"同伴行"必须拿到同一个值**。

至此仓库里**没有任何可从 SQL 触达的未实现功能**。
唯一剩下的 `TopNPerGroupExecutor` 是死代码：`src/optimizer/` 里没有任何规则
会生成 `TopNPerGroupPlanNode`，只有 `executor_factory.cpp` 留了个永不命中的
`case`。实现它等于写死代码，因此没做。
